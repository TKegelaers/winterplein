...
- [x] T1: Create `Season` entity with all properties and `GetMatchdays()` method
- [x] T2: Add validation logic to `Season` (empty name, EndDate <= StartDate, EndHour <= StartHour) (blockedBy: T1)
- [x] T3: Create `SeasonDto` record with `Id` (int), `Matchdays` and `MatchdayCount` (blockedBy: T1)
- [x] T4: Create `CreateSeasonRequest` and `UpdateSeasonRequest` records
- [x] T5: Add `Players` collection to `Season` entity — private `List<Player> _players`, exposed as `IReadOnlyList<Player> Players`; `AddPlayer(Player)` throws if null or already enrolled; `RemovePlayer(int playerId)` throws if not found or if removal would leave fewer than 4 players (blockedBy: T1)
- [x] T6: Add `List<PlayerDto> Players` to `SeasonDto` record (blockedBy: T3)
- [x] T7: Create `AddSeasonPlayerRequest(int PlayerId)` record in `Winterplein.Shared/DTOs/`
